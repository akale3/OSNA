...
def bfs(graph, root, max_depth):
    ...
    while nodeQueue:
        ...
        currentNodeNeighbors = graph.neighbors(currentNode)
        for neighborNode in currentNodeNeighbors:
            parentList = []
            parentList.append(currentNode)
            if neighborNode not in visitedNodes:
                distance = node2distances[currentNode] + 1
                if distance <= max_depth:
                    node2distances[neighborNode] = distance
                    nodeQueue.append(neighborNode)
                    visitedNodes.append(neighborNode)
                    nestedNeighbors = graph.neighbors(neighborNode)
                    for nestedNeighborNode in nestedNeighbors:
                        if nestedNeighborNode in node2distances:
                            if node2distances[nestedNeighborNode] == node2distances[currentNode]:
                                if nestedNeighborNode not in parentList:
                                    parentList.append(nestedNeighborNode)
                    node2parents[neighborNode] = parentList
                    node2num_paths[neighborNode] = len(parentList)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for nodes in node2distances:
        originValues[nodes] = 1
    edgesCreditValue = dict()
    for node in sorted(node2distances, key=node2distances.get, reverse=True) or []:
        if node != root:
            noOfParents = len(node2parents[node])
            if noOfParents > 1:
                for parent in node2parents[node]:
                    tup = (node, parent)
                    sortedTup = sorted(tup)
                    storedTup = (sortedTup[0], sortedTup[1])
                    edgesCreditValue[storedTup] = float(originValues[node] / noOfParents)
                    originValues[parent] = float(originValues[parent] + edgesCreditValue[storedTup])
            else:
                parent = str(node2parents[node][0])
                tup = (node, parent)
                sortedTup = sorted(tup)
                storedTup = (sortedTup[0], sortedTup[1])
                edgesCreditValue[storedTup] = float(originValues[node])
                originValues[parent] = float(originValues[parent] + edgesCreditValue[storedTup])
    return edgesCreditValue
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        bfsResult = bfs(graph, node, max_depth)
        bottomUpResult = bottom_up(node, bfsResult[0], bfsResult[1], bfsResult[2])
        for edge in bottomUpResult:
            if edge in betweenness:
                betweenness[edge] = betweenness[edge] + bottomUpResult[edge]
            else:
                betweenness[edge] = bottomUpResult[edge]
    for element in betweenness:
        betweenness.update({element: betweenness.get(element) / 2})
    return betweenness
...
def partition_girvan_newman(graph, max_depth):
    ...
    ['D', 'E', 'F', 'G']
    """
    copiedGraph = nx.Graph(graph)
    betweennessResult = sorted(approximate_betweenness(copiedGraph, max_depth).items(), key=lambda x: (-x[1], x[0]))
    components = list(nx.connected_component_subgraphs(copiedGraph))
    while len(components) == 1:
        edgeNodes = betweennessResult.pop(0)[0]
        copiedGraph.remove_edge(edgeNodes[0], edgeNodes[1])
        components = list(nx.connected_component_subgraphs(copiedGraph))
    return components
...
def volume(nodes, graph):
    ...
    for node in nodes:
        for graphNode in graph.nodes():
            if graph.has_edge(node, graphNode):
                tup1 = (node, graphNode)
                tup2 = (graphNode, node)
                if tup1 not in edgesList or tup2 not in edgesList:
                    edgesList.append(tup1)
                    edgesList.append(tup2)
                    volumeCount += 1
    return volumeCount
...
def cut(S, T, graph):
    ...
    for nodeInS in S:
        for nodeInT in T:
            if graph.has_edge(nodeInS, nodeInT):
                cutCount += 1
    return cutCount
...
def norm_cut(S, T, graph):
    ...
    """
    cutCount = cut(S, T, graph)
    firstPart = float(cutCount / volume(S, graph))
    secondPart = float(cutCount / volume(T, graph))
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        normCutValue = norm_cut(components[0], components[1], graph)
        scoreMaxDepths.append((depth,normCutValue))
    return scoreMaxDepths
...
def make_training_graph(graph, test_node, n):
    ...
    """
    copiedGraph = nx.Graph(graph)
    sortedNeighborNodes = sorted(copiedGraph.neighbors(test_node))
    if len(sortedNeighborNodes) < n:
        n = len(sortedNeighborNodes)
    for x in range(n):
        copiedGraph.remove_edge(sortedNeighborNodes[x], test_node)
    return copiedGraph
...
def jaccard(graph, node, k):
    ...
    """
    scores = []
    nodeNeighbors = set(graph.neighbors(node))
    for n in graph.nodes():
        if n != node and not graph.has_edge(node, n):
            tup = (node, n)
            nNeighbors = set(graph.neighbors(n))
            jaccardCoefficient = len(nodeNeighbors.intersection(nNeighbors)) / len(nodeNeighbors.union(nNeighbors))
            tup1 = (tup, jaccardCoefficient)
            scores.append(tup1)
...
def path_score(graph, root, k, beta):
    ...
    """
    scores = []
    bfsResult = bfs(graph, root, float("inf"))
    shortestPaths = bfsResult[0]
    noOfShortestPath = bfsResult[1]
    for node in graph.nodes():
        if node != root and not graph.has_edge(root, node):
            similarity = float(beta ** shortestPaths[node] * noOfShortestPath[node])
