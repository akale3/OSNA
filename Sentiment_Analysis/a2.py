        ...
        for minFrequency in min_freqs:
            for L in range(0, len(feature_fns) + 1):
                for subset in combinations(feature_fns, L):
                    if subset:
                        X, vocabs = vectorize([tokenize(d,punctuation) for d in docs], subset, minFrequency, vocab=None)
...
def plot_sorted_accuracies(results):
    ...
    for entries in results:
        accuracyList.append(entries['accuracy'])
    accuracyList.sort()
...
def mean_accuracy_per_setting(results):
    ...
    """
    settingDict = defaultdict(lambda: 0)
    settingCountDict = defaultdict(lambda: 0)
    for setting in results:
        if setting['punct'] == True:
            settingDict['punct=True'] = settingDict['punct=True'] + setting['accuracy']
            settingCountDict['punct=True'] = settingCountDict['punct=True'] + 1
        else:
            settingDict['punct=False'] = settingDict['punct=False'] + setting['accuracy']
            settingCountDict['punct=False'] = settingCountDict['punct=False'] + 1
        settingDict['min_freq=' + str(setting['min_freq'])] = settingDict['min_freq=' + str(setting['min_freq'])] + setting['accuracy']
        settingCountDict['min_freq=' + str(setting['min_freq'])] = settingCountDict['min_freq=' + str(setting['min_freq'])] + 1
        finalFeatureName = 'feature='
        for featureName in setting['features']:
            finalFeatureName = finalFeatureName + str(featureName).split()[1] + ' '
        settingDict[finalFeatureName] = settingDict[finalFeatureName] + setting['accuracy']
        settingCountDict[finalFeatureName] = settingCountDict[finalFeatureName] + 1
    listOfAaccurracy = [(v, k) for k, v in settingDict.items()]
    meanAccuracyList = []
    for item in listOfAaccurracy:
        meanAccuracy = item[0] / settingCountDict[item[1]]
...
def top_coefs(clf, label, n, vocab):
    ...
    Returns:
      ...
      given class label.
    """
    vocabElements = np.array([k for k, v in sorted(vocab.items(), key=lambda tup: tup[1])])
    coef = clf.coef_[0]
    if label == 1:
    ...
    top_coef_terms = vocabElements[top_coef_ind]
    top_coef = abs(coef[top_coef_ind])
    result = [x for x in zip(top_coef_terms, top_coef)]
    sortedResult = sorted(result, key=lambda tup: tup[1], reverse=True)
    return sortedResult
